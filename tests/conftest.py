...
def temp_enterprise_ide_complete(tmp_path, monkeypatch):
    ...
    Both files live in the same directory (the SSO cache directory),
    matching real-world layout where KIRO_CREDS_FILE points into ~/.aws/sso/cache/.

    Returns tuple: (creds_file_path, device_reg_file_path)
    """
    # Create SSO cache directory
    sso_cache_dir = tmp_path / ".aws" / "sso" / "cache"
    sso_cache_dir.mkdir(parents=True, exist_ok=True)

    # Create credentials file in SSO cache directory
    creds_file = sso_cache_dir / "kiro-auth-token.json"
    creds_data = {
        "accessToken": "enterprise_access_token",
        "refreshToken": "enterprise_refresh_token",
        "expiresAt": "2099-01-01T00:00:00.000Z",
    ...
    # Create device registration file in same directory
    device_reg_file = sso_cache_dir / "abc123def456.json"
    device_reg_data = {
        "clientId": "enterprise_client_id_12345",
        "clientSecret": "enterprise_client_secret_67890",
        "region": "us-east-1",
