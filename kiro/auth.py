...
class KiroAuthManager:
    ...
    def _load_credentials_from_file(self, file_path: str) -> None:
        ...
        try:
            ...
            if "clientIdHash" in data:
                ...
                self._load_enterprise_device_registration(
                    self._client_id_hash, path.parent
                )
    ...
    def _load_enterprise_device_registration(
        ...
        Enterprise Kiro IDE uses AWS SSO OIDC authentication. Device registration is stored
        in the same directory as the credentials file: {cache_dir}/{clientIdHash}.json

        Args:
            client_id_hash: Client ID hash used to locate the device registration file
            cache_dir: Directory containing the SSO cache files
        """
        try:
            device_reg_path = cache_dir / f"{client_id_hash}.json"
